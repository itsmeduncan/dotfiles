...
- **Pi (earendil-works/pi):** `pi/models.json` (local LM Studio + Tailscale Ollama `darksaber` providers; Anthropic models are built-in, key resolved at runtime via `!op read` from 1Password) and `pi/settings.json` (local-first default model) are symlinked to `~/.pi/agent/` by the agents phase. Secrets stay out of the repo — `~/.pi/agent/auth.json` is machine-local and never tracked.
