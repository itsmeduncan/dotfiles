...
- **`claude/settings.json`** — Pre-approved tool permissions, `defaultMode: auto` (continuous execution), status line, hooks (`PostToolUse` auto-format on Edit/Write, `Stop` commonplace capture), `effortLevel: high`, `theme: dark-ansi`, `tui: fullscreen`, `skipAutoPermissionPrompt`, `agentPushNotifEnabled`, and enabled plugins (`swift-lsp`, `frontend-design`, `superpowers`, `skill-creator`, `feature-dev`, `claude-code-setup`, `figma`, `rust-analyzer-lsp`, `clangd-lsp`, plus `prime` from the `arthai` marketplace).
...
- **`claude/skills/` → `agents/skills/`** (symlink) — Workflow skills shared across Claude, Codex, opencode, and pi. The directory is a mix of vendored submodules and symlinked sibling repos. Use `ls agents/skills/` for the live inventory. Categories:
  ...
  - **Vendored Cloudflare / web-platform skills** (plain directories, not submodules): `cloudflare` (umbrella platform skill), `agents-sdk`, `wrangler`, `workers-best-practices`, `durable-objects`, `sandbox-sdk`, `cloudflare-email-service`, `cloudflare-one`, `cloudflare-one-migrations`, `turnstile-spin`, `web-perf`.
